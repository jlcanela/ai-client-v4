...
import { FetchHttpClient, HttpClient, HttpClientResponse, Headers } from "effect/unstable/http";
...
const program = Effect.gen(function* () {
  ...
  const OutputSchema = Schema.Struct({
    feeling: Schema.String.pipe(Schema.annotate({
      description: "The detected feeling or emotion",
    })),
      weather: Schema.String.pipe(Schema.annotate({
  ...
  const [body, response] = yield* client.createResponse({
    model: "gpt120b",
    messages: [
